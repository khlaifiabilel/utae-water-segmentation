...
class DatasetDownloader:
    ...
    def download_dataset(self) -> dict[str, Any]:
        ...
        try:
            ...
            dataset = load_dataset(
                self.dataset_name, cache_dir=self.cache_dir, trust_remote_code=True
            )
    ...
    def save_dataset_info(self, dataset, output_file: str = "dataset_info.yaml"):
        ...
        for split_name in dataset:
            split_data = dataset[split_name]
            split_info = {"num_samples": len(split_data), "features": {}}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download IBM Granite UKI flood detection dataset"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
    ...
    downloader = DatasetDownloader(cache_dir=args.cache_dir, data_dir=args.data_dir)
