...
class FloodDataPreprocessor:
    ...
    def analyze_data_structure(self, dataset) -> dict:
        ...
        for split_name in dataset:
            ...
            sample = split_data[0]
            split_analysis = {"num_samples": len(split_data), "features": {}}
            ...
            for feat_name, feat_info in split_analysis["features"].items():
                if "shape" in feat_info:
                    logger.info(
                        f"  {feat_name}: {feat_info['shape']} ({feat_info['dtype']})"
                    )
    ...
    def resize_data(
        self, data: np.ndarray, target_size: tuple[int, int], mode: str = "bilinear"
    ) -> np.ndarray:
        """
    ...
    def process_sample(self, sample: dict, channel_mapping: dict) -> dict:
        ...
        if "s1_key" in channel_mapping:
            ...
            s1_resized = self.resize_data(
                s1_normalized, self.target_size, mode="bilinear"
            )
        ...
        if "s2_key" in channel_mapping:
            ...
            s2_resized = self.resize_data(
                s2_normalized, self.target_size, mode="bilinear"
            )
        ...
        if "mask_key" in channel_mapping:
            ...
            mask_resized = self.resize_data(
                mask_data[None, ...], self.target_size, mode="nearest"
            )[0]
    ...
    def process_dataset(self, dataset, max_samples_per_split: int | None = None):
        """
        ...
        self.analyze_data_structure(dataset)

        # Identify data channels from first sample
        first_sample = dataset[next(iter(dataset.keys()))][0]
        ...
        for split_name in dataset:
            ...
            for i in tqdm(range(n_samples), desc=f"Processing {split_name}"):
                try:
                    ...
                    logger.error(
                        f"Error processing sample {i} in {split_name}: {error}"
                    )
                    continue
            ...
            logger.info(
                f"Saved {len(processed_samples)} processed samples to {split_file}"
            )
    ...
    def visualize_samples(self, num_samples: int = 3):
        ...
        for split_name in ["train", "validation", "test"]:
            ...
            for i in range(min(num_samples, 5)):
                ...
                plt.tight_layout()
                plt.savefig(
                    viz_dir / f"{split_name}_sample_{i}.png",
                    dpi=150,
                    bbox_inches="tight",
                )
                plt.close()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess IBM Granite UKI flood detection dataset"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Create sample visualizations"
    )
